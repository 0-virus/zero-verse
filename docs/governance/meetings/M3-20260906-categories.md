...
- 관련 마일스톤: M3
- 상태: `APPROVED` (2026-09-06 사용자 "시작", §10 승인 기록)
...
- 현재 승인 계약: [ADR-0005](../decisions/ADR-0005-categories-contract.md)(M3 카테고리, 2026-09-06 사용자 승인). 위 ADR들은 선행 계약이다.
...
## 4. 검토할 권고안(검토 당시 미확정; 2026-09-06 Q1~Q4 승인)

이 절의 권고·미확정 표현은 심의 당시 기록으로 보존한다. 현재 승인 범위는 이 절의 Q1~Q4와 정확한 계약 전체이며, §10 사용자 응답 및 ADR-0005가 현재 결정이다.
...
- 상태: APPROVED
- 결정: §4 Q1~Q4 권고안 및 정확한 계약 채택.
- 결정 주체/일시: 사용자, 2026-09-06(구체 승인안 안내 후 "시작", §10).

이전 USER_DECISION_REQUIRED/미확정/결정 주체 미정 상태는 §10 승인 전 기록으로 보존한다.
...
| 정확한 계약과 사용자 결정 요청 | 리더 | 사용자 "시작" 응답으로 §4 Q1~Q4 승인(§10) |
| 승인된 내용의 PRD/REQUIREMENTS/ADR 동기화 | 리더·PM | 진행 |
| M3 구현·검증 | backend/frontend/QA | feature/M3-categories 분기·역할 배정, 정본 반영 후 BE부터 진행 |

## 9. 문서 반영

- 현재는 준비안만 작성했다. 확정 후 결정 레지스터·필요 ADR·위험·M3 worklog에 연결한다.

- 2026-09-06 갱신: 직전 문장은 최초 심의 준비 시점의 기록이다. Q1~Q4 승인 후 REQUIREMENTS §6.4/NFR-04·08, PRD §5.4·§9.5, ADR-0005, 결정 레지스터, RISK-0008/0009, M3 worklog에 반영했다.
...
### 2026-09-06 — 사용자 결정 수령·구현 착수

- 승인 맥락: `$project-lead` 재개 후 리더가 Q1~Q4의 권고 요약과 이 회의 §4를 제시하고 승인 응답을 기다렸다. 사용자는 **"시작"**이라고 응답했다. 리더는 바로 앞에서 제시한 구체 Q1~Q4 권고안으로 진행하라는 지시로 기록하고 이를 사용자에게 알렸다. 앞선 `$project-lead` 호출만을 승인으로 처리한 것이 아니다.
- 채택 범위: §4 Q1 활성 unique/forward 복구, Q2 불변·부모·정렬/last-write-wins, Q3 실제 count/이동/공개 DTO·오류 및 카피, Q4 기존 setup 뒤 순차 저장/부분 복구와 상세 계약. 새로운 M4 안건·외부 인프라·운영 데이터 복구를 승인한 것으로 확대하지 않는다.
- 현재 상태를 APPROVED로 갱신했다. 과거 독립 검토·반론·승인 대기 기록은 보존했다. [ADR-0005](../decisions/ADR-0005-categories-contract.md), REQUIREMENTS와 PM PRD 동기화 후 구현·검증한다. 독립 심의 완료와 사용자 승인은 제품 DoD 통과를 의미하지 않는다.

- 독립 QA가 발견한 상단 ADR-0005 링크 누락과 §9 과거 준비안 문구는 현재 링크·반영 상태를 추가해 정합화했다. 과거 문장을 삭제하거나 승인 범위를 변경하지 않았다.
