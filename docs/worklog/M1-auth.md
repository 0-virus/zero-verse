...
### 2026-07-26 · Codex 2차 리뷰 반영 — blocking 2건

**Verdict: `NEEDS REWORK`**. 1차 지적 10건 중 9건은 해소 확인, **#9는 부분 해소**로 재지적됐고
**새 결함 1건(#11)** 이 나왔다. 둘 다 "구현은 맞는데 테스트가 그것을 검증하지 않는다"는 종류다.

#### #11 — 중복키 회귀 테스트가 fake-pass (블로킹)

바로 앞 커밋에서 내가 추가한 `nicknameValueNamedEmailIsNotMisclassified`가 **순차 요청**이었다.
순차 가입은 `UserRegistrar`의 **선조회**에서 `USER_002`가 먼저 던져져 `mapConstraintViolation`에
아예 도달하지 못한다. 즉 **매핑을 옛날 코드로 되돌려도 이 테스트는 통과**한다. 오분류를 고쳤다고
기록해 놓고 그 수정을 지키지 못하는 테스트를 붙인 셈이다.

| 조치 | 내용 |
|---|---|
| 매핑 로직 분리 | `AuthService`의 private 메서드 → `auth/support/RegisterConstraintMapper`. 이 경로는 **동시 가입에서만** 도달하므로 통합 테스트로는 특정 메시지 형태를 결정적으로 재현할 수 없다. 별도 타입으로 빼야 검증 가능해진다 |
| 단위 테스트 | `RegisterConstraintMapperTest` 12케이스 — 값이 `email`·`url_slug`인 닉네임 충돌, 스키마 한정자 유무, `uk_*` 명시 제약, FK 위반, 미지의 제약, null 메시지, 예외 래핑 |
| 통합 테스트 교정 | `ConcurrentRegisterTest`의 해당 케이스를 **동시 요청**으로 바꿨다. 선조회를 통과해 DB 제약까지 가는 경로를 재현한다 |
| **뮤테이션 검증** | 매핑을 옛 `contains` 방식으로 되돌려 실행 → **3건 FAILED**(`'email'` 닉네임, `'url_slug'` 닉네임, 예외 래핑). 테스트가 실제로 이 결함을 잡는다는 것을 확인한 뒤 복원했다 |

#### #9 — refresh 파싱 실패 관측에 테스트 없음 (블로킹)

로그 분기(만료 debug / 서명 warn / 형식 warn)는 구현돼 있었지만 **어떤 테스트도 그것을 실행하지
않았다**. 응답이 전부 `AUTH_003`이라 **로그가 유일한 구분 수단**인데, 그 로그가 사라지거나 등급이
뒤바뀌어도 아무도 알아채지 못하는 상태였다.

`RefreshTokenObservabilityTest`(6케이스) 추가 — Logback `ListAppender`를 실제로 붙여 검증한다.

- 만료 → `DEBUG`(정상 흐름이 경보가 되면 안 된다), 서명 위조 → `WARN`, 형식 오류·Access 토큰
  오용 → `WARN` + `kind=MalformedJwtException`, 쿠키 없음/공백 → `DEBUG`.
- **누출 검증**: 4가지 실패 토큰 각각에 대해 로그 전문에 토큰 원문·**8자 슬라이딩 윈도우 조각
  전부**·시크릿이 없고 `ThrowableProxy`가 null인지(예외 객체를 통째로 싣지 않는지) 확인한다.
  JJWT 예외 메시지는 토큰 조각을 포함할 수 있어 예외를 그대로 로깅하면 여기서 걸린다.
- DB에 닿지 않는다 — 파싱 실패는 저장소 조회 이전에 끝나므로 repository는 mock이다.

**검증**: BE **138 tests** / 16 클래스 · 0 skipped · 0 failures · 0 errors.

#### 2026-07-26 · Codex 3차 리뷰 반영 — blocking 1건

#11은 해소 확인. **#9의 누출 검증이 여전히 약하다**는 지적을 받았다.

- 문제: segment **전체**만 `doesNotContain`으로 비교해, `substring(0, 16)`처럼 **앞자리 일부를
  로깅**하면 통과한다. 워크로그에 "8자 이상 조각을 검증한다"고 적어 놓고 실제로는 그러지 않았다.
- 수정: 각 segment의 **8자 슬라이딩 윈도우 전부**를 뽑아 어느 위치의 조각이든 걸리게 했다.
- **뮤테이션 검증**: 서명 실패 로그에 `rawRefreshToken.substring(0, 16)`을 실어 실행 →
  `logsNeverLeakTokenMaterial` **FAILED**. 이전 단정이었다면 통과했을 케이스다. 확인 후 복원했다.

**재검증**: BE **138 tests** · 0 skipped · 0 failures · 0 errors.
...
- 2026-07-26 · 1차 반영 후 **자체 발견 1건**(중복키 오분류) 수정 + 테스트 4건 보강.
- **2026-07-26 · Codex 2차 리뷰 — Verdict `NEEDS REWORK`**. 1차 10건 중 9건 해소 확인,
  #9(refresh 관측 로그 테스트 부재) 부분 해소로 재지적, 신규 #11(중복키 회귀 테스트 fake-pass).
  처리 내역은 [개발 기록 「Codex 2차 리뷰 반영 — blocking 2건」](#2026-07-26--codex-2차-리뷰-반영--blocking-2건) 참조.
  두 건 모두 **테스트가 구현을 지키지 못하는** 유형이었다 — 지적이 타당하다.
