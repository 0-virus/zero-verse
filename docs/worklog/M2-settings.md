...
### 2026-07-27 23:50 · Codex 7차 리뷰(완주분) 반영 — blocking 4건 + 비블로킹 1건

7차는 두 번 판정 직전에 죽었고 세 번째 실행에서 완주했다. 그 사이 확정된 blocking 1건은 이미 반영한
상태였고, 완주분이 **blocking 4건 + non-blocking 1건**을 냈다.

**[blocking] HIGH 심의가 사용자 승인 없이 구현됐다 — 코드가 아닌 절차 결함.**
2026-07-26에 등급 `HIGH` 심의를 "사용자 포괄 위임"을 근거로 Claude가 `APPROVED`로 바꾸고 진행했다.
AGENTS.md와 `docs/governance/README.md`는 **`LOW`만 자동 승인**하고 나머지는 사용자 승인 후 진행하도록
규정한다. "중대하지 않은 결정은 권장안대로 진행하라"는 포괄 위임을 **심의 등급 규칙보다 우선 적용한 것이
잘못**이었다. ADR-0004가 결정 주체를 "사용자"로 적어 둔 것도 실제와 달랐다.

2026-07-27 사용자에게 Q1·Q2·Q3을 각각 제시해 **명시 승인**을 받았다(Q1=`A`, Q2=`A`, Q3=`B` — 심의
권고와 동일). 회의록·ADR-0004·DECISION-REGISTER를 실제 승인 주체·날짜로 정정하고 위반 사실을 정정
기록으로 남겼다. 판단이 권고와 일치해 되돌릴 구현은 없었으나, 달랐다면 되돌려야 했다.

**[blocking] initial-setup 부분 성공 후 `BLOG_004`에서 갇혔다.** 서버는 커밋했는데 응답이 유실되거나
`refreshUser`만 실패하면 클라이언트는 미완료로 남고, 재시도하면 `BLOG_004`가 온다. 기존 코드는 메시지만
띄워서 `SetupGuard`가 다른 화면을 계속 `/blog/setup`으로 되돌렸다 — 새로고침 전엔 빠져나올 수 없었다.
다른 탭에서 완료한 경우도 같다. `BLOG_004`를 **상태 불일치 신호**로 다뤄 `refreshUser()` + `/blogs/me`
재조회 후 자기 블로그로 보낸다. 복구 조회마저 실패하면 새로고침 안내를 남긴다.
기존 테스트가 "이동하지 않는다"는 **잘못된 동작을 고정**하고 있어 교체했다.

**[blocking] 설정 GET이 한 번 실패하면 카드가 페이지 수명 동안 잠겼다.** 실패 경로가 `loaded`를 올리지
않는데 effect는 `userId`가 바뀔 때만 다시 돈다. 6차의 `fieldset` 잠금이 일시적인 5xx 한 번으로 영구
잠금이 됐다 — **잠금이 사용자를 가두면 안 된다.** 조회를 `useCallback`으로 분리하고 재시도 버튼을
**잠긴 `fieldset` 바깥**에 뒀다(안에 두면 버튼까지 비활성이라 의미가 없다).

**[blocking] 31자 slug가 `VALIDATION_001`로 차단됐다.** 6차에서 하한만 풀고 `@Size(max = 30)`을 남겨
31자 이상이 도메인에 닿지 못했다. slug 형식 오류는 `BLOG_003`이라는 NFR-04·ADR-0004 계약 위반이다.
**절반만 고친 것이 경계 하나에서만 오류코드를 가르는 결과**를 낳았다. slug의 Bean Validation을 완전히
제거해 형식·길이·예약어를 모두 도메인이 판정하게 했다.

**[non-blocking] 동시성 테스트가 stale read를 보장하지 않았다.** latch가 서비스 **진입 전**에만 동기화해,
한쪽이 SELECT와 커밋을 끝낸 뒤 다른 쪽이 읽으면 매핑이 잘못돼도 통과할 수 있었다. `CyclicBarrier`로
**두 트랜잭션이 모두 읽은 뒤** 쓰도록 강제하고, `Blog`의 `updateInfo` ↔ `initialSetup` 경합도 추가했다.
`@DynamicUpdate` 제거 뮤테이션에서 **두 테스트 모두 FAILED** — 결정적 증거를 확보했다.

**검증**: BE **346 tests** / 53 suites · 0 failures · 0 errors · **0 skipped**(`cleanTest test`, 10m13s,
343 → 346) · FE **241 tests** / 21 파일 · 0 failures(239 → 241) · lint exit 0 · build 성공.
...
- 2026-07-27 · **심의 등급이 `LOW`가 아니면 포괄 위임으로 승인을 대체할 수 없다.** "중대하지 않은 결정은
  권장안대로 진행하라"는 위임은 **등급 규칙 위에 있지 않다**. `HIGH` 심의를 위임으로 채택하고 문서에는
  결정 주체를 "사용자"로 적은 것이 7차 리뷰에서 머지 차단 사유가 됐다. 등급이 `LOW`를 넘으면 반드시
  사용자에게 안건별로 제시하고 명시 승인을 받는다.
- 2026-07-27 · **잠금은 사용자를 가두면 안 된다.** 조회 실패로 `fieldset`이 잠겼는데 재시도 수단이 잠금
  안에 있으면 페이지를 다시 열기 전엔 빠져나올 수 없다. **보호 장치를 넣을 때는 그 장치가 실패했을 때의
  탈출 경로를 같이 만든다** — `BLOG_004` 갇힘도 같은 종류였다.
- 2026-07-27 · **절반만 고치면 경계에서 계약이 갈린다.** slug 하한만 풀고 상한을 남겨 31자에서만 오류코드가
  달라졌다. 검증 책임을 도메인으로 옮기기로 했으면 **그 필드의 검증을 전부** 옮긴다.
